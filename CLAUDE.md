...
- **French.** All UI strings, seed data, error messages, and email templates are French. DB stores translatable fields as plain TEXT. Web strings live in `apps/*/messages/fr.json` (next-intl as a string loader); **buyer-mobile strings are plain French literals in the Dart code — the gen-l10n/`app_fr.arb` layer was removed 2026-06-22** (seller-mobile still on gen-l10n pending a follow-up). See Rule 1.
...
1. **All user-facing text is French — written directly, not localized.** **buyer-mobile** has no localization layer: write French string literals straight in the Dart widgets. The `flutter_localizations`/gen-l10n stack (`app_fr.arb`, generated `AppLocalizations`, `l10n.yaml`) was removed 2026-06-22 — do **not** add an ARB/l10n key for new strings, and do not re-introduce gen-l10n. Web still reads `apps/*/messages/fr.json` via next-intl (string-loader only); seller-mobile still has gen-l10n. **Both are planned follow-up removals** — until then, match the surrounding file's existing pattern. (Framework `GlobalMaterialLocalizations`/`GlobalWidgetsLocalizations` stay pinned to `fr` — that's Flutter rendering built-in widgets, not our layer.)
