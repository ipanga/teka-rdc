...
12. **Authentication is email + password — everywhere, all roles.** No phone-OTP, no SMS-based login, no Google OAuth, no magic links. Users carry an `authProvider` on `User`; the only value created by current code paths is `EMAIL_PASSWORD`. Role mapping:
    - **Buyers** — self-service register at `POST /v1/auth/register/buyer`. Legacy buyers migrate via `POST /v1/auth/buyer/{migrate-check, migrate-link-email, setup-password}` (phone lookup → setup link to email → set password).
    - **Sellers** — self-service register at `POST /v1/auth/register/email` → admin approval. Legacy sellers migrate via `/v1/auth/seller/{migrate-check, migrate-link-email, setup-password}` (no OTP step).
    - **Admins** — seeded out-of-band (see `docs/deployment.md § 5b`).

    Every role logs in at `POST /v1/auth/login/email`. Password reset is shared (`/v1/auth/password-reset/{request, confirm}`).

    Removed endpoints (return 404): `/v1/auth/otp/request`, `/v1/auth/otp/verify`, `/v1/auth/register` (phone+OTP), `/v1/auth/login` (phone+OTP), `/v1/auth/login/google`, `/v1/auth/otp/request-email`. The `PHONE_OTP` and `GOOGLE` enum values stay on `AuthProvider` for historical accounts only — no code path creates new ones, and no UI exposes them.

13. **Phone is delivery contact, not auth.** Phone is collected only at address / delivery-contact / seller-profile surfaces — never at login or registration. Users type 9 digits (or 10 with leading `0`); `+243` is added by the system. Single source of truth: `normalizeDrcPhone()` in `packages/shared/src/utils/phone.ts` (web) and `apps/buyer-mobile/lib/core/utils/phone.dart` (Flutter). Backend DTOs that store phone enforce `^\+243\d{9}$`. `User.phone` is `String? @unique` — email-only buyers have `null`.

14. **SMS is notification-only.** `SmsService` + the Orange/AT/Mock providers exist only to send order status notifications, payment confirmations, and admin broadcasts. There is no SMS code path for authentication. Order notifications skip users with `phone = null`.
