...
- **French-first UX:** All UI copy, emails, SMS, error messages, and notifications default to French. Use i18n from day one (next-intl for Next.js, flutter_localizations for Flutter). Store all translatable strings in structured locale files, never hardcode text.
...
12. **Auth providers.** Users carry an `authProvider` on the `User` model: `PHONE_OTP` (buyers/admins), `EMAIL_PASSWORD` (sellers/admins), `GOOGLE` (all roles). All three paths terminate in the same `generateTokens` helper so cookie semantics and refresh-token replay detection are identical. Sellers are **required** to use email/password + Google (phone OTP path returns `409 SELLER_MIGRATION_REQUIRED`). See `docs/architecture.md` for the migration flow.
