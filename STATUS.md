# Status — 2026-09-11 (production `main` `c4bd594`; `develop` `4161dcb`, synchronized. **P3 PR 1 — foreign product characteristics — COMPLETE.** `2026-09-11_foreign_product_specifications.sql` applied via the *Apply prod migration* workflow (run 34570557921): 6 characteristics repointed onto their own leaf, 3 invalid rows deleted, 6 deliberately untouched, `product_specifications` 350 → 347. **« Type : Bière » is gone from the Johnnie Walker page.** 3 planned repoints correctly did nothing — the chemises product already held identical canonical rows and the unique-constraint guard refused them. `update()` now reconciles foreign characteristics so a seller can no longer be trapped with a row no form renders. The earlier 2026-09-10/11 taxonomy work (categories, characteristics, brand links) remains complete. ## Active initiative: None — P2/P3 taxonomy backlog audited and ranked, not started.)
