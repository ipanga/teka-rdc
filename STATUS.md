# Status — 2026-09-11 (production `main` `c4bd594`; `develop` synchronized + P2 PR A. **P1 foreign product characteristics CLOSED** (migration run 34570557921; `product_specifications` 350 → 347; « Type : Bière » gone from the whisky page). **P2 IN PROGRESS** — read-only audit proved the 2026-06-24 refactor reused every `13000000-` subcategory id, stranding **52 attributes across 25 live intermediate categories** (6 with ACTIVE-product dependencies → P3; 46 latent → PR B deactivation). **PR A merged** (`ec0631a`, four structural invariants + read-only audit script). **PR B open, not merged**: retire the 46 latent legacy characteristics onto an inactive holding category — data-only, nothing deleted, 172 historical specifications keep their foreign key. Next after PR B: PR C (evidence-backed DRC brands), then PR D (taxonomy:diff/apply). ## Active initiative: P2 taxonomy structural remediation.)
