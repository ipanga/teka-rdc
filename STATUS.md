# Status — 2026-09-11 (production `main` `e84d9a3`; `develop` synchronized. **P2 PR A + PR B COMPLETE IN PRODUCTION.** `2026-09-11_retire_legacy_intermediate_attributes.sql` applied via *Apply prod migration* (run 34577377907): **46 legacy characteristics retired onto an inactive holding category, attributes on live intermediate nodes 52 → 6**, `product_attributes` unchanged at 584, all 172 historical specifications intact, the 6 ACTIVE-dependent P3 rows untouched, live tree unchanged at 7/37/150. P1 foreign characteristics closed earlier (350 → 347). Also merged: the Admin `softDelete` guard (#761) — a category holding live products can no longer be deleted. **PR C open, not merged**: 15 evidence-backed DRC beverage brands (Brasimba, HQ Lubumbashi + Bralima, brewery in Lubumbashi) for « Bières », « Eau » and « Sodas ». **PR D open, not merged**: `taxonomy:diff` / `taxonomy:apply` — read-only reconciliation that classifies differences into additive drift, needs-a-decision, Admin-managed and historical; `apply` writes a reviewable migration FILE and never touches a database. ## Active initiative: P2 taxonomy structural remediation — PR C next.)
