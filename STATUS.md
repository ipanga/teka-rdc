# Status — 2026-09-11 (production `main` `c91ecad`; `develop` synchronized. **P2 COMPLETE**; **P3-1 and P3-2 COMPLETE IN PRODUCTION**. P3-1: deleting a characteristic still used by products now answers a French 400 instead of a bare 500. P3-2: the three duplicate shirt characteristics removed (migration run 34598641280, `product_specifications` **347 → 344**); the shirt now renders Taille=M, Couleur=Bleu, Matière=Coton exactly once each and no seller value was lost. Invariant allowlist **9 → 6**. **Note:** `taxonomy:diff` judgement stayed at **6, not 3** — it counts attribute ROWS on live intermediate nodes, and P3-2 deliberately kept those rows. The three « Cuisine » attributes now have **zero live-product references**, so retiring them onto the holding category (as PR B did for 46) is a natural small follow-up that would bring judgement to 3. **P3-3 COMPLETE IN PRODUCTION** (release #774 `7fde4e4`, deploy run 34604210836): admin category-tree transitions are now validated — a node carrying historical characteristics may not lose its last child (resurrection), a leaf that serves characteristics or holds products may not gain a first child (blinding), a subtree may not be re-parented past 3 levels, and a category may not be moved under itself or its own descendant. All four were ACCEPTED before, proven by a development runtime probe. Validation only: no schema, migration or data change, and `taxonomy:diff` production output is unchanged (judgement 6, historical 1). Admin category CRUD — create, rename, metadata, activate/deactivate, re-parent, characteristic CRUD/reorder, brand links, safe deletion — is unchanged and covered by a regression suite. **P3-4a COMPLETE IN PRODUCTION** (migration run 34614497729): the Galaxy A14 « Mémoire interne » repointed onto the canonical « Stockage » of its own Android leaf with the value "16Go" preserved byte-for-byte and the row id unchanged; « Mémoire interne » retired to the holding category with its 4 historical specifications intact. Buyer PDP shows « Stockage = 16Go » exactly once; the seller form is pre-filled and editable; no stranded characteristic remains. `taxonomy:diff` judgement **6 → 5**, allowlist **6 → 5**, ledger 45 → 46, all table totals unchanged. **P3-4b COMPLETE IN PRODUCTION** (migration run 34623237888): « Taille », « Couleur » and « Matière » retired from « Cuisine » onto the holding category — 3 attribute rows written, **0 specification rows**, all 8 historical specifications byte-for-byte preserved, ledger 46 → 47, holding 47 → 50, `taxonomy:diff` judgement **5 → 2**. **P3-4c COMPLETE IN PRODUCTION** (migration run 34630448908) (dedicated `IRON` template; « Type » appended last so 382 canonical ids stay stable — 1 added, 0 moved; Aspirateurs deliberately excluded from the `APP_GENERIC` sharing trap). `taxonomy:diff` now **additive 0 · judgement 1 · historical 1** — the only remaining judgement case is the oil « Type ». **P3-4d COMPLETE IN PRODUCTION** (migration run 34642979050): canonical « Type » on « Huiles » with three oil types only (Vinaigre/Sel/Épices/Sauce excluded as Condiments-family values); the specification kept its id, product and the value "Huile végétale" byte-for-byte; the legacy attribute retired to the holding category with all 4 historical specifications — **including both « Sel » rows** — untouched. Dedicated `OIL` template, so none of the six sibling BEVERAGE leaves received it (verified live). Ledger 48 → 49, attributes 585 → 586, specifications 344 → 344, holding 51 → 52, live-intermediate attributes **1 → 0**. **POST-P3 ALLOWLIST MAINTENANCE COMPLETE** (PR #793, merged 2026-09-12, `dd48758`): the invariant-2 allowlist now holds exactly **3** entries. P3-4d had made the oil row `5bf39dfd…` non-foreign — its attribute and its product sit on the same leaf, so the invariant returns before ever reading the allowlist — leaving a permit that excused nothing, and the entry was retired (4 → 3). The other three (`rt7ibz` "Lait en poudre", `vnkqce` "Savon de lessive", `d3k7ei` "Blender") are **individually proven load-bearing** against production — removing any one of them surfaces exactly one violation — and stay as unresolved catalogue decisions, each needing a catalogue answer rather than a repoint. That PR also corrected the `attribute.referencedHistorical` detail text, which claimed the referencing products are soft-deleted while the code never reads product state. Code and tests only: no migration, no schema change, no production data change. **No further P3 implementation work is required.** **With it the P3 TAXONOMY CLEANUP IS COMPLETE:** `taxonomy:diff` now reports **additive 0 · judgement 0 · historical 1** (exit 0) — production and `taxonomy-data.ts` agree in full, reached without ever running `taxonomy:apply`; the single remaining finding is the 80 deliberately preserved historical characteristics. **Still open (deliberately, not defects):** the 117+2 preserved rows, the 9 pre-ledger migration artifacts, and development-DB drift. **BUYER ANDROID RELEASED TO PLAY INTERNAL TESTING (2026-09-12)** — the last P1 release blocker, now closed. `com.tootiye.teka` **0.1.8 (versionCode 10)** built from `main` `c91ecad` and uploaded to the **`internal`** track by *Release mobile AAB* [run 34690164811](https://github.com/ipanga/teka-rdc/actions/runs/34690164811), after the `android-play` environment was approved by hand. Google's own response is the evidence, not the green check: « Preparing mapping … version code 10 for upload », « Updating track 'internal' », « Successfully finished the upload to Google Play », « buyer: com.tootiye.teka uploaded to 'internal' ». `validate_only false` · `release_status completed` · store listing untouched (metadata, images, screenshots and changelogs all skipped) · R8 mapping uploaded, so Play crash reports de-obfuscate. **versionCode 10 was NOT previously consumed** — Play accepted it, so no version bump was needed and none was made. **Why it had been missing:** never a configuration gap — every buyer signing secret and the `android-play` environment already existed. The `play` job was added in `6994b6e` (2026-09-10 20:24), *after* the last buyer AAB build (`eace209a`, 09:53 that morning) and *before* the seller builds (`e5cce072`), so buyer was simply never re-dispatched. **The mobile release matrix is now complete:** Buyer iOS TestFlight `0.1.8+10` · Buyer Android Play internal `0.1.8+10` · Seller iOS TestFlight `0.1.10+12` · Seller Android Play internal `0.1.10+12`. **Still open, deliberately separate (P3):** buyer-mobile **real-device validation** — the cart/order/address sequence, the product-card/PDP final device pass and the sticky-header device matrix. Publishing to internal testing is **not** device validation, and none was performed. A public/production Play release is **not** part of this and stays a manual Play Console promotion. **FINAL RECONCILIATION PASS (2026-09-12)** — the cleanup/release cycle is closed and feature development can resume. **Confirmed COMPLETE, do not rediscover as missing:** buyer search analytics (`SearchQuery` carries term · termNormalized · resultCount · cityId · source · intent, with `intent=SUBMIT|SUGGESTION` so autocomplete keystrokes are not logged as searches), admin search analytics (list · summary · trending · breakdown · csv), admin sales analytics (`status='DELIVERED' AND deletedAt IS NULL`, CANCELLED excluded, RETURNED excluded by the lifecycle; breakdowns by product · category · seller · town · day), CSV export (all report + analytics routes, UTF-8 BOM, and formula-injection guarded by `common/utils/csv.util.ts` for `=`/`+`/`-`/`@`/TAB/CR including a trimmed check), search autocomplete and typo tolerance (Postgres FTS `websearch_to_tsquery('french', f_unaccent(…))` + pg_trgm `word_similarity`, ranked real-over-demo → ts_rank → trigram → recency), synonym expansion, and the seller catalogue: characteristics are **leaf-only and server-owned** — verified live that « Chemises » serves exactly Taille/Couleur/Matière and no cosmetics or oil characteristic reaches it, while both seller surfaces read the same `/v1/browse/categories/{id}/attributes` endpoint (no divergent hardcoded lists). Brands are category-scoped (341 links, 68 active, **0 duplicate identities**, and **all 150 live leaves carry « Autre »**). **Fixed in this pass:** `returnedAt` on a forced return and buyer-web's double search navigation (PR #796 `aefef1d`), synonym admin CRUD (PR #797 `275dfeb`), v33 splash residue (PR #798 `577e93e`). **P1 release blockers: 0. Outstanding production migrations: 0.** Buyer iOS TestFlight `0.1.8+10` · Buyer Android Play internal `0.1.8+10` · Seller iOS TestFlight `0.1.10+12` · Seller Android Play internal `0.1.10+12`. **Deliberately deferred, not defects:** buyer-mobile real-device validation (P3, needs the owner's hardware), the seller/admin `?status=`-on-mount staleness (cosmetic, and a correct fix must reconcile `useSearchParams` with the pages' deliberate `window.history.replaceState` usage across two apps — too large for its benefit), lint/format debt, CLAUDE.md headroom, the dev-DB search-object drift (**never run `pnpm db:push` against dev**), PostHog dashboard verification, 9 Dependabot PRs, PR #673, and the P5 business decisions (alcohol policy, the three intentional invariant-2 catalogue entries, brand-coverage expansion). **BUYER MOBILE 0.1.8+11 ON BOTH STORES (2026-09-12)** — released from `main` `7167d62` (release PRs #800 `7fb20cc` + #803 `7167d62`; deploy runs 34714993524 and 34715894671, both success; no migration, ledger unchanged at 49 and every table count identical before and after). **Buyer iOS → TestFlight**, run [34716309564](https://github.com/ipanga/teka-rdc/actions/runs/34716309564): App Store Connect app `6787126945` accepted the binary and **finished processing** — « Successfully finished processing the build 0.1.8 - 1789243959 for IOS », then « Successfully distributed build to Internal testers ». Store-visible build is **0.1.8 (1789243959)**: `release-mobile-ipa.yml` deliberately overrides the pubspec build number with a Unix epoch (`date -u +%s`) so CFBundleVersion is always monotonic, so the marketing version comes from pubspec and the build number does not. Group: fastlane reported « distributed to 'Teka Buyer test team' » but also warned « not listed under 'Teka Buyer test team' yet, though the group takes all builds » — **confirm the group listing in App Store Connect**. **Buyer Android → Play internal**, run [34716318861](https://github.com/ipanga/teka-rdc/actions/runs/34716318861): « Preparing mapping … version code 11 for upload », « Updating track 'internal' », « Successfully finished the upload to Google Play », « buyer: com.tootiye.teka uploaded to 'internal' » — `validate_only false`, `release_status completed`, `skip_upload_aab false`, listing untouched. **versionCode 11 accepted**, so the pubspec bump `+10 → +11` was what Android needed. **Seller Mobile deliberately NOT rebuilt or bumped** — diff from its last Play build (`e5cce072`) to released `main` is 0 files and from its last TestFlight build (`315040bf`) is one *test* file, which never ships; it stays at **0.1.10+12**, already current on both tracks. **Mobile release matrix, all four current:** Buyer iOS `0.1.8` (1789243959) TestFlight · Buyer Android `0.1.8+11` Play internal · Seller iOS `0.1.10+12` TestFlight · Seller Android `0.1.10+12` Play internal. **Still open and unchanged: real-device validation (P3)** — reaching a testing track means installable, NOT device-tested. Nothing here was validated on hardware. No public/production release on either store: App Store production review was not submitted, and Play promotion beyond internal testing remains a manual Play Console step. ## Active initiative: None.)
...
- ~~**`SearchSynonym` has no admin CRUD.**~~ **RESOLVED 2026-09-12 (PR #797, `275dfeb`).** Full admin
  CRUD at `v1/admin/reports/search/synonyms` — list with active/total counts, read, create, update,
  activate, deactivate, delete; `@Roles('ADMIN')`, `UuidParam` on every id. Two rules the UI cannot
  enforce alone are in the service: deactivate rather than delete (`isActive=false` is the off switch
  `BrowseService` already honours, and it keeps the note), and no two ACTIVE groups may share a term
  (overlap makes expansion order-dependent; refused with the term named, folded for accents/case).
  Search behaviour is unchanged — `getSynonymGroups()` still reads `isActive` on its 60s cache.
- ~~**`forceStatusChange()` has the same gap for `returnedAt`**~~ **FIXED 2026-09-12 (PR #796,
  `aefef1d`).** Forcing a status to RETURNED now stamps `returnedAt` when it is not already set, the
  same fill-never-overwrite pattern #628 used for `deliveredAt`. Pinned by a mirror invariant suite.
- ~~**buyer-web fires two `router.push` calls for one Enter.**~~ **FIXED 2026-09-12 (PR #796,
  `aefef1d`).** The `!open` branch of `onKeyDown` now calls `e.preventDefault()` itself, so the native
  submit no longer fires alongside it: one push, one history entry. A new
  `search-autocomplete.test.tsx` pins exactly one push per Enter and per direct submit — reverting the
  fix turns it red, which also confirms the double-push was real rather than theoretical.
...
- ~~**buyer-mobile `values-v33` splash themes keep `windowDrawsSystemBarBackgrounds">true`**~~
  **RESOLVED 2026-09-12 (PR #798, `577e93e`)** — both v33 sidecars cleaned, and the splash test now
  asserts `windowDrawsSystemBarBackgrounds` and `windowFullscreen` are absent from **all six** launch
  themes so the asymmetry cannot return. seller-mobile was already clean. Original diagnosis: while the four
