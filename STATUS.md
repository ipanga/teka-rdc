# Status — 2026-09-11 (production `main` `1c3e6ee`; `develop` `72de05e`, synchronized. **P2 TAXONOMY REMEDIATION COMPLETE IN PRODUCTION.** PR A structural invariants, PR B legacy-characteristic retirement (52 → 6 on live intermediate nodes), the Admin category soft-delete guard, PR C evidence-backed DRC beverage brands (**brands 55 → 70, links 326 → 341**, migration run 34586775240) and PR D `taxonomy:diff`/`taxonomy:apply`. **Acceptance test passed: `taxonomy:diff` against production went from 30 additive findings to 0**, leaving only the 6 known P3 intermediate characteristics and 76 intentionally preserved historical rows. P1 foreign characteristics closed earlier. **Open and untouched: P3** (6 ACTIVE-dependent characteristics, 3 shirt duplicates, 117+2 unreferenced rows, `deleteAttribute` raw FK message), **9 pre-ledger migration artifacts** absent from `_manual_migrations` and NOT executed, and **development-database drift** (295 additive / 39 judgement) NOT modified. ## Active initiative: None — P3 backlog audited and ranked, not started.)
