# Status — 2026-09-11 (production `main` `9115580`; `develop` synchronized. **Taxonomy attribute P2 CLOSED** — `2026-09-10_taxonomy_attribute_backfill.sql` applied in production on 2026-09-10, `product_attributes` 560 → 584, all six new leaves and the canonical « Déodorants » verified live. **In flight: `fix/taxonomy-brand-links`** — the same source-of-truth gap left those six leaves with ZERO `brand_categories` rows, so the seller brand dropdown is empty (not even « Autre »). Additive migration generated from `taxonomy-data.ts`; not merged, not deployed. Android Play uploads are automated since #745 (`android-play` approval gate, internal track). No mobile version bumped, no store build triggered.)
