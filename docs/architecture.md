...
**Per-user in-app notifications (2026-06-18).** A separate, per-USER feed (distinct from the global admin inbox above): `UserNotificationService` writes to the `UserNotification` model (`userId` FK, `UserNotificationType` enum). Every read/write is scoped to `userId` — `markRead` filters `{ id, userId }` so a user can never touch another's rows (no IDOR). It backs **seller product approval/rejection** (`SellerNotificationService.notifyProductApproved/Rejected`), **new orders** (type `ORDER`) and, since 2026-09-04, **payout approved / paid / rejected** (type `PAYOUT`, `entityType 'payout'` → the seller's owner-scoped payout detail; `docs/payouts.md`): each persists an in-app row **and** fans out push-primary + email-fallback, instead of the old push-only. Read API: `GET /v1/seller/notifications` (+`/unread-count`) and `PATCH .../:id/read` (+`/read-all`), scoped to `@CurrentUser`. Surfaced by a notification bell on seller-web (sidebar header) + a `/notifications` center on seller-mobile (home app-bar bell + unread badge). Reusable for buyers later. Migration `2026-06-18_user_notifications.sql`.
...
| **Category** | **3-level** self-tree via parentCategoryId (Catégorie → Sous-catégorie → **Type de produit**), plain-TEXT French name, emoji, unique slug. Since the 2026-06-24 refactor the 7-cat / 35-sub / ~145-type tree is the only active one; older nodes are deactivated **and soft-deleted**. Products link to the **leaf (product type)**. See "Marketplace taxonomy + brands" below. |
...
### Marketplace taxonomy + brands (3-level, refactored 2026-06-24)
The catalog is a **3-level taxonomy** — **Category → Subcategory → Product Type** (**7 categories → 35 subcategories → ~145 product types**) — built by **reusing the self-referential `Category` tree**: product types are `Category` nodes whose parent is a subcategory (UUID range `16000000-`). **There is no separate ProductType table** — the tree, attribute system, brand links, breadcrumb (3-level include + unroll), and browse child-expansion (self + 2 levels) were all already depth-agnostic, so the blast radius was a data re-seed + a recursive facet count roll-up. Defined once in `apps/api/prisma/taxonomy-data.ts`, seeded as the **only active** tree (older nodes are deactivated **and soft-deleted**, FK-safe; existing products remapped by old subcategory name — removed cats had 0 real products). The 7 (fixed display order): Supermarché · Téléphones & Accessoires · Électronique · Électroménager · Mode · Beauté & Santé · Maison & Cuisine. **Construction & Bricolage + Automobile & Moto were removed.** **Products link to the leaf (product type).** Attributes + brands attach **per product type**. **Attributes are LEAF-ONLY and are NOT inherited**: `GET /v1/browse/categories/:id/attributes` returns `[]` for any node that still has a non-deleted child, and otherwise returns only that node's OWN rows — it never walks the parent chain. That is deliberate: intermediate nodes still carry legacy pre-refactor rows (« Mode > Homme » holds *Type de peau*, « Électroménager > Climatisation » holds *Pointure*), and serving them is what put cosmetics attributes on a men's shirt. Brands DO resolve per linked category (`/v1/brands?categoryId=`); brand library always includes **"Autre"**. **Condition is the `NEW`/`USED` enum, NOT an attribute.** Admin manages all 3 levels via the polymorphic category endpoints + `PATCH /v1/admin/categories/reorder`; sellers pick the product-type leaf via the searchable selector (web + mobile); buyers drill Category→Subcategory→Product Type (chips, web + mobile) and the sitemap indexes all 3 levels. Deterministic id ranges: categories/subcategories `13000000-`, product types `16000000-`, attributes `14000000-`, brands `15000000-`.
...
- **Characteristics are LEAF-ONLY, and the invariant is enforced** — `BrowseService.getCategoryAttributes` returns `[]` for any category that still has a non-deleted child, and otherwise serves only that category's OWN rows. **It never walks the parent chain**, so a leaf does not inherit its subcategory's or department's attributes. Verified against production: « Mode > Homme » returns 0 attributes, its leaf « Chemises » returns 3.

  > **Invariant.** Seller-selectable product characteristics belong to canonical **live leaf** categories, unless an explicitly documented exception exists.
...
### Search engine (FTS + synonyms + ranking + logging)
Product search runs on **Postgres full-text search** in `BrowseService` (`GET /v1/browse/products?search=`). A STORED generated column `search_vector = to_tsvector('french', f_unaccent(title || ' ' || description))` (GIN-indexed) is matched with `websearch_to_tsquery('french', f_unaccent(q))`; **accent-insensitive** via the IMMUTABLE `public.f_unaccent()` wrapper. **Typo tolerance** uses pg_trgm `word_similarity` (`<%`) on unaccented title (+ GIN trgm indexes on brand/category names for autocomplete). **Synonyms** (Req: zero-code-change) live in the admin-editable **`SearchSynonym`** table (`terms[]`, `isActive`); at search time the query is expanded — any group it hits ORs its other terms into the tsquery (gsm → portable/smartphone/telephone; frigo → réfrigérateur; tv; pc/laptop). Groups are cached 60s; terms are sanitized to tsquery lexemes (injection-safe). **Ranking** order-by: `isDemo → ts_rank → word_similarity → in-stock → unitsSold → avgRating → recency` (relevance dominates; popularity/quality break ties). Every first-page search is logged fire-and-forget to **`SearchQuery`** (term, normalized, resultCount, cityId) — powering `GET /v1/browse/search/popular` (top non-zero terms, 7d, city-scoped) + zero-result reports. **Autocomplete** `GET /v1/browse/search/suggestions` returns products + categories (all 3 levels) + brands; `@Throttle(40/10s)`. Migration: `2026-06-24_search_synonyms_and_logging.sql` (idempotent). Web + mobile add **recent searches** (localStorage / shared_preferences) + popular chips in the empty/zero-result states. `/recherche` stays **noindex** (category pages remain the indexable surface); analytics: `search_performed` + `zero_results` (PostHog, PII-scrubbed).
...
### Notifications & broadcasts (2026-06-23)
A production notification platform built on a **generic, user-scoped feed** (`UserNotification`: `type`, `title`, `body`, `entityType`/`entityId` deep-link, `readAt`) + **FCM push** (`PushService.sendToUser`). One platform serves admin broadcasts today and order/delivery/wishlist/back-in-stock later with no redesign.
- **Admin broadcasts** (`NotificationBroadcast`, admin `/v1/admin/broadcasts`, `@Roles('ADMIN')` + send `@Throttle`): target **all buyers** (segment) **or specific buyers** (`recipientIds`) + optional **linked product** (`productId` → `PRODUCT_PROMO`, deep-links to the PDP). On send, **fan-out on write** — the loop `createMany`s one `UserNotification` per recipient (the durable in-app record; push/email opt-outs only gate *delivery*, never the feed) and sends push `data` = `screen:product-details+productId` (promo) or `screen:notifications` (generic).
- **Buyer feed** — unified `GET/PATCH /v1/notifications` (current authed user, any role; the seller `/v1/seller/notifications` alias is untouched) backing the **buyer-web bell + `/notifications` page** (client islands; the page is `noindex`; 60s unread-count polling) and the **buyer-mobile Notification Center + AppBar unread badge**. Deep-link: a product notification opens the PDP (web: bare `/{id}` 308→canonical; mobile: `NotificationRouter` `product-details`/`notifications` → go_router). FCM receipt/tap (foreground/background/terminated) was already implemented.
- **Admin surface:** the existing broadcasts page, extended into the **Centre de notifications** (audience toggle + buyer autocomplete + product typeahead + live preview + created-by/recipients audit). **Migration:** `2026-06-23_broadcast_notifications.sql` (enum `BROADCAST`/`PRODUCT_PROMO` + `productId`/`recipientIds`; idempotent). Analytics: server `broadcast_sent`; client `notification_opened`. iOS push deferred (no GoogleService-Info.plist yet).
