...
- **D1 — Two axes or one.** Recommended: keep `applicationStatus` (account approval, gates selling) and add `verificationStatus` (badge). Sellers keep selling unverified; the badge is earned separately and revocable without suspending the account. Alternative: promote approval itself to « Vérifié » (no new state, but then revoke = suspend and the current one-photo review becomes the badge's meaning).
- **D2 — `Commune.isActive`.** Recommended yes (mirrors `City`, stops hard deletes from nulling seller locations). Also: seed communes for **Likasi** (launch market, currently un-applicable) — needs the official list; do not invent it.
- **D3 — Evidence rule for the badge.** Recommended: `company` → RCCM **and** Identification Nationale (NIF optional under OTHER); `individual` → one government identity document (carte d'électeur, passport, permis, CNI). Admin still decides; the rule only defines when the status may move to PENDING_REVIEW.
- **D4 — Commune mandatory for legacy sellers?** Recommended: prospective only. The two legacy sellers stay editable; the profile screens show « Commune » as required only when the seller changes city or saves a commune.
- **D5 — Badge meaning and transition.** Recommended label « Vérifié » with helper text « Teka a vérifié les documents fournis par ce vendeur » (never government endorsement). Shipping PR 5 makes the two legacy sellers and every new seller **unverified** until admins review them; decide whether to pre-verify the two document-bearing sellers via an explicit admin action after PR 4 (not a backfill).
- **D6 — Where the badge appears.** Recommended: PDP seller block only (web + mobile) now; product cards and a future seller page later.
- **D7 — Retention.** Recommended: rejected/superseded documents purged 90 days after review; all documents destroyed on account anonymisation; accepted documents kept while verified; the audit rows (no file) kept forever.
- **D8 — Roles.** Recommended: `ADMIN` only for approve/reject/revoke and document viewing (today's boundary); `SUPPORT` sees status only.

**Next exact step:** decisions D1–D8, then PR 1 (Commune foundation) on `feat/seller-commune-foundation`.
