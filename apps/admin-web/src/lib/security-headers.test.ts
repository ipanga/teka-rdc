...
describe('admin-web security headers (D4)', () => {
  ...
  it('images: Cloudinary delivery + the signed private-download host used for seller documents', () => {
    const i = directive(buildPortalCsp(nonce, prod), 'img-src');
    expect(i).toEqual(["'self'", 'data:', 'blob:', 'https://res.cloudinary.com', 'https://api.cloudinary.com']);
  });
