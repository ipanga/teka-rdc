...
// PostHog client bootstrap for admin-web.
//
// Initialized once, gated on NEXT_PUBLIC_POSTHOG_KEY_ADMIN_WEB — when the key
...
//
// Analytics traffic is proxied through the admin-web origin /ingest (see
// next.config.ts rewrites) so ad-blockers don't drop events. In production
// NEXT_PUBLIC_BASE_PATH is empty (the app serves at the admin.teka.cd root —
// see deploy.yml), so /ingest behaves exactly as on buyer-web. Pageviews are
...
export function PostHogProvider({ children }: { children: React.ReactNode }) {
  useEffect(() => {
    const key = process.env.NEXT_PUBLIC_POSTHOG_KEY_ADMIN_WEB;
    ...
    posthog.init(key, {
      ...
      autocapture: true,
      // D4 (2026-09-06): no session replay on the admin portal — identity,
  ...
  // Tie analytics identity to the logged-in admin — id + role only, never
