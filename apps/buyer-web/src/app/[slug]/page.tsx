...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (product) {
    ...
    // Build the description with a fixed tail (category + seller + brand) and
    // budget what's left for the product description, so the seller name never
    // gets cut mid-word at the 160-char SEO limit.
    const MAX_DESC = 160;
    const tail =
  ...
  // Fallback when the API fetch fails (timeout, invalid slug, transient
  // outage). Emit a complete OG/Twitter card so social previews never
  // collapse to a bare domain title.
  const fallbackDesc =
  ...
  return {
    ...
    twitter: {
      ...
      title: 'Teka RDC',
      description: fallbackDesc,
      images: ['https://teka.cd/og-default.png'],
    },
  };
}
