...
export default function BuyerSetupPasswordPage() {
  return (
    <Suspense fallback={null}>
      <BuyerSetupPasswordPageInner />
    </Suspense>
  );
}

function BuyerSetupPasswordPageInner() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const res = await apiFetch<{ user: User }>('/v1/auth/buyer/setup-password', {
        method: 'POST',
