...
describe('OrdersService.cancelOrder (buyer) — D7', () => {
  it('unpaid COD: payment flipped inside the cancellation transaction, status log + stock restore kept, payment_failed emitted once', async () => {
    ...
    expect(paymentsService.failCodPaymentOnCancellation).toHaveBeenCalledWith(expect.objectContaining({ id: 'o1', paymentMethod: PaymentMethod.COD }), tx);
    expect(tx.orderStatusLog.create).toHaveBeenCalledTimes(1);
    expect(tx.product.update).toHaveBeenCalledTimes(1);
    expect(tx.order.update.mock.calls[0][0].data).toMatchObject({ status: OrderStatus.CANCELLED, cancellationReason: 'Changé d’avis', cancelledBy: 'b1' });
  ...
  it('no flip (payment not PENDING / not COD) → no payment_failed event', async () => {
    const { service, analytics } = makeService({ ...codPending(), paymentStatus: PaymentStatus.COMPLETED }, false);
  ...
  it('a second cancellation is refused before any write (idempotent)', async () => {
    ...
    expect(paymentsService.failCodPaymentOnCancellation).not.toHaveBeenCalled();
  });
