...
/// Consumes the 24h JWT from the seller-setup email link and sets the password.
/// Reached via deep-link (`teka-seller://setup-password?token=...`) or in-app
/// navigation after the migrate flow when the user is already in the app.
class SetupPasswordScreen extends ConsumerStatefulWidget {
  final String? token;

  const SetupPasswordScreen({super.key, this.token});
...
class _SetupPasswordScreenState extends ConsumerState<SetupPasswordScreen> {
  ...
  Future<void> _submit() async {
    ...
    try {
      await ref
          ...
          .setupSellerPassword(widget.token!, _passwordController.text);
      if (mounted) context.go('/');
    } on DioException catch (e) {
      setState(() {
        _errorMessage = e.response?.data?['error']?['message'] ??
            'Lien de configuration invalide ou expire.';
      });
    } catch (_) {
  ...
  Widget build(BuildContext context) {
        ...
        body: Center(
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                const Text('Lien invalide ou expire.', textAlign: TextAlign.center),
                const SizedBox(height: 16),
                ElevatedButton(
                  onPressed: () => context.go('/auth/migrate'),
                  child: const Text('Recommencer la migration'),
                ),
              ],
            ),
          ),
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Definir votre mot de passe'),
        backgroundColor: Colors.transparent,
        foregroundColor: TekaColors.foreground,
        elevation: 0,
      ),
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: [
                const SizedBox(height: 16),
                Text(
                  'Choisissez un mot de passe pour votre compte vendeur.',
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    child: _isLoading
                        ...
                        : const Text('Definir mon mot de passe'),
                  ),
                ),
              ],
            ),
