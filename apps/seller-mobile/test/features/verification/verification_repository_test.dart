...
/// interceptor refreshes — hence the marker — but cannot replay a multipart
/// body), the second 200.
void main() {
  test(
    ...
    final dio = Dio()
      ..interceptors.add(InterceptorsWrapper(onRequest: (options, handler) {
        if (options.method == 'POST') {
          ...
          if (posts == 1) {
            options.extra[authRefreshedExtraKey] = true;
  ...
  test('a 401 the interceptor could not refresh behind is not retried',
      () async {
    var posts = 0;
    final dio = Dio()
      ..interceptors.add(InterceptorsWrapper(onRequest: (options, handler) {
    ...
    expect(posts, 1, reason: 'the session was not refreshed: no second row');
  });
