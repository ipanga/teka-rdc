...
class _FailingNotificationsRepo extends NotificationsRepository {
  _FailingNotificationsRepo() : super(Dio());
...
class _NotificationsFixture extends NotificationsNotifier {
  _NotificationsFixture(
    NotificationsState initial, {
    NotificationsRepository? repo,
  }) : super(repo ?? _NotificationsRepo()) {
    state = initial;
  }
}
...
void main() {
  ...
  test('a failed refresh over a cached list does not become a load-more error',
      () async {
    final notifier = _NotificationsFixture(
      NotificationsState(
        items: [
      ...
      repo: _FailingNotificationsRepo(),
    );
    addTearDown(notifier.dispose);
    ...
    // Fully loaded feed: loadMore is a no-op and must not leave stale errors.
    await notifier.loadMore();
    expect(notifier.state.loadMoreError, isNull);
  });

  NotificationsState cachedList({String? error, String? loadMoreError}) =>
      NotificationsState(
        items: [
          NotificationModel(
            id: 'n1',
  ...
  testWidgets('a failed refresh over a cached list shows no load-more footer',
      (tester) async {
    final notifier = _NotificationsFixture(
      cachedList(error: 'Service momentanément indisponible'),
    );
    await _pump(
      tester,
      const NotificationsScreen(),
    ...
    expect(find.text('Première'), findsOneWidget);
    expect(find.text('Réessayer de charger la suite'), findsNothing);
    expect(tester.takeException(), isNull);
  });

  testWidgets('a pagination failure shows the load-more footer',
      (tester) async {
    final notifier = _NotificationsFixture(
      cachedList(loadMoreError: 'Service momentanément indisponible'),
    );
    await _pump(
      tester,
      const NotificationsScreen(),
    ...
    expect(find.text('Réessayer de charger la suite'), findsOneWidget);
    expect(tester.takeException(), isNull);
  });
